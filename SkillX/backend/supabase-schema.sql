...
-- Persisted client-visible events generated by successful milestone submissions.
...
create index if not exists idx_notifications_client_created on notifications(client_wallet, created_at desc);
...
begin
  ...
  insert into notifications (client_wallet, job_id, milestone_id, freelancer_wallet, message)
  select j.client_wallet, j.job_id, v_milestone.milestone_id, p_freelancer_wallet,
    'Freelancer submitted milestone work for project ' || j.title
  from jobs j where j.job_id = v_milestone.job_id;
